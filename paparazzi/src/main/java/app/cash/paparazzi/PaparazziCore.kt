...
import app.cash.paparazzi.agent.registerInterceptors
...
class PaparazziCore @JvmOverloads constructor(
  ...
  override fun init(testName: TestName) {
  ...
  override fun <V : View> inflate(@LayoutRes layoutId: Int): V =
  ...
  @JvmOverloads
  override fun snapshot(view: View, name: String?) {
  ...
  override fun unsafeUpdateConfig(
    deviceConfig: DeviceConfig?,
    theme: String?,
    renderingMode: RenderingMode?
  ) {
  ...
  private fun forcePlatformSdkVersion(compileSdkVersion: Int) {
    ...
    val versionClass = try {
      PaparazziCore::class.java.classLoader.loadClass("android.os.Build\$VERSION")
